...
-- 日志级别
local ngx_ALERT        = ngx.ALERT   -- 报警
local ngx_CRIT         = ngx.CRIT    -- 严重，系统故障， 触发运维告警系统
local ngx_ERR          = ngx.ERR     -- 错误，业务不可恢复性错误
local ngx_WARN         = ngx.WARN    -- 提醒，业务中可忽略错误
local ngx_INFO         = ngx.INFO    -- 信息，业务琐碎日志信息，包含不同情况判断等
...
----------------------------------------------------------------
--- name:     execute_cache_warmup
--- function: 执行缓存查找
--- param:    kong_config     string    kong的配置文件         M
--- return:   true       返回命中的值
---           nil     表示未命中
----------------------------------------------------------------
local function execute_cache_warmup(kong_config)
  ...
  -- ngx.worker.id是openresty的一个接口，返回当前 Nginx 工作进程的一个顺序数字（从 0 开始）
  -- 此处即判断是否刚刚启动worker进程
